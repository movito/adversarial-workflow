...
**Created**: 2025-11-01
**Updated**: 2026-02-18 (deprecate built-in evaluators, use library evaluators)
...
**Evaluator**: External AI via adversarial-workflow (library evaluators)

> **DEPRECATION NOTICE (2026-02-18)**: The built-in evaluators (`adversarial evaluate`,
> `adversarial proofread`, `adversarial review`) are **deprecated**. Use library
> evaluators instead — they support multiple providers (OpenAI, Google, Anthropic,
> Mistral), configurable models, and role-specific evaluation.
>
> **Quick migration**:
...
> - `adversarial proofread <file>` → no direct replacement yet; use `adversarial gemini-flash <file>` with appropriate prompt
>
...
adversarial architecture-reviewer-fast <input-file>    # Fast check (Gemini Flash)

# Discovery
adversarial list-evaluators              # Show all available evaluators

# Output is always in .adversarial/logs/<input-name>--<evaluator-name>.md

# System Commands
adversarial --version                    # Check CLI version
adversarial check                        # Validate setup and dependencies

# DEPRECATED (built-in, use library evaluators above instead):
