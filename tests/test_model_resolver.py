...
class TestModelResolverLegacyFallback:
    ...
    def test_resolve_fallback_on_unknown_family(self):
    ...
    def test_resolve_fallback_on_unknown_tier(self):
        """Resolver falls back to legacy when tier unknown."""
        config = EvaluatorConfig(
            name="test",
            description="Test",
            model="gpt-4o",
    ...
    def test_model_requirement_takes_precedence(self):
        """model_requirement is used when both fields present."""
        config = EvaluatorConfig(
            name="test",
            description="Test",
        ...
        resolver = ModelResolver()
        model_id, api_key_env = resolver.resolve(config)
        # Should use model_requirement, not legacy
        assert "claude" in model_id.lower()
        assert api_key_env == "ANTHROPIC_API_KEY"
...
class TestModelResolverWarnings:
    """Tests for ModelResolver warning behavior."""

    def test_fallback_warning_includes_evaluator_name(self):
        """Fallback warning includes the evaluator name."""
        config = EvaluatorConfig(
            name="my-evaluator",
            description="Test",
            model="gpt-4o",
            ...
            prompt="Test",
            output_suffix="TEST",
            model_requirement=ModelRequirement(family="unknown", tier="tier"),
        )
        resolver = ModelResolver()
        with pytest.warns(UserWarning, match="my-evaluator"):
            resolver.resolve(config)

    def test_fallback_warning_includes_legacy_model(self):
        """Fallback warning includes the legacy model being used."""
        config = EvaluatorConfig(
            name="test",
            description="Test",
            model="gpt-4o",
        ...
        resolver = ModelResolver()
        with pytest.warns(UserWarning, match="gpt-4o"):
            resolver.resolve(config)
