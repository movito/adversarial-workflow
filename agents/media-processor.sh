...
# Universal Media Processor Agent
...
if [[ -f "$ROLES_CONFIG" ]]; then
    ROLE_JSON=$(sed -n '/\"media-processor\":/,/},/p' "$ROLES_CONFIG")
    ...
    ROLE_NAME="Media Processor"
    ROLE_ICON="🎵"
    SYSTEM_PROMPT="You are a media processor agent. Focus on: media file analysis, format validation, processing pipelines, and performance optimization."
fi
...
if [[ -f "$CONTEXT_DIR/agent-handoffs.json" ]]; then
    ...
    MEDIA_HANDOFF=$(echo "$HANDOFFS" | sed -n '/\"media-processor\":/,/},/p')
    if [[ -n "$MEDIA_HANDOFF" ]]; then
        PROJECT_CONTEXT+="Role Context: $MEDIA_HANDOFF. "
        echo -e "${GREEN}Role-Specific Context Loaded${NC}"
    fi
fi

echo
echo -e "${BLUE}Launching Claude Code with Media Processor specialization...${NC}"
echo
