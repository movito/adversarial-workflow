...
# Universal Document Reviewer Agent
...
if [[ -f "$ROLES_CONFIG" ]]; then
    ROLE_JSON=$(sed -n '/\"document-reviewer\":/,/},/p' "$ROLES_CONFIG")
    ...
    ROLE_ICON="📖"
    SYSTEM_PROMPT="You are a document reviewer agent. Focus on: reviewing technical documentation for clarity and completeness, validating specifications against requirements, and improving document quality."
fi
...
if [[ -f "$CONTEXT_DIR/agent-handoffs.json" ]]; then
    ...
    DOCUMENT_HANDOFF=$(echo "$HANDOFFS" | sed -n '/\"document-reviewer\":/,/},/p')
    if [[ -n "$DOCUMENT_HANDOFF" ]]; then
        PROJECT_CONTEXT+="Role Context: $DOCUMENT_HANDOFF. "
        echo -e "${GREEN}Role-Specific Context Loaded${NC}"
    fi
fi

echo
echo -e "${BLUE}Launching Claude Code with Document Reviewer specialization...${NC}"
echo
