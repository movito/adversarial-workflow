...
**IMPORTANT**: This project uses a Python virtual environment. Always use the correct commands:

```bash
# Running tests (use venv pytest, NOT system python)
...
# python3 -m pytest ...    # pytest not in system path
# pytest ...               # May use wrong Python
```
...
## PR Review Comments (Automated Tools)

**IMPORTANT**: If a PR URL is provided (e.g., `https://github.com/movito/adversarial-workflow/pull/XX`), you MUST check for automated review comments from tools like:
- **CodeRabbit** - AI code reviewer
- **Cursor Bugbot** - Bug detection

These tools often catch issues that manual review might miss.

### How to Check PR Comments

```bash
# Get PR review comments
gh api repos/movito/adversarial-workflow/pulls/XX/comments --jq '.[] | "File: \(.path)\nLine: \(.line)\nAuthor: \(.user.login)\nBody: \(.body)\n---"'

# Get PR reviews (summary comments)
gh api repos/movito/adversarial-workflow/pulls/XX/reviews --jq '.[] | "Reviewer: \(.user.login)\nState: \(.state)\nBody: \(.body)\n---"'
```

### Triage Automated Findings

When reviewing automated comments:

| Priority | Action |
|----------|--------|
| **Bugs** (logic errors, incorrect behavior) | Include as HIGH finding |
| **Unused code** (parameters, variables) | Include as MEDIUM finding |
| **Style/formatting** (f-strings, etc.) | Include as LOW or skip |
| **False positives** | Document why it's not applicable |

Include relevant automated findings in your review report under a "## Automated Tool Findings" section.
...
- Fetch PR comments from GitHub (via `gh api` or WebFetch)
