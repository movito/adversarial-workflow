...
The `git checkout -b` creates a feature branch for the task. The `project start` command:
...
2. **Verify CI passes**: Run `./scripts/core/verify-ci.sh <branch> --wait` (see above)
...
- **Complete task without verifying CI/CD passes on GitHub via `./scripts/core/verify-ci.sh`**
